import React, { useEffect, useState } from 'react';
import Modal from 'react-modal';
import '../../CSS/lesson-agenda.css';
import 'swiper/swiper-bundle.css';
import 'swiper/css';
import 'swiper/css/navigation';
import 'swiper/css/pagination';
import SEO from '../../Components/SEO.js';
import GrammarExplanation from '../../Components/GrammarExplanationComps/tenseexplanation.js';
import Grammarexercises from '../../Components/GrammarExerciseComps/tenseexercisescomp.js';
import Reading from '../../Components/ReadingComps/readingcomponent.js';
import readingcompData from '../Reading-Exercises/Data/readingcomp.js'
import CategoryMatch from '../../Utils/vocabmatch.js';
import GapMatch from '../../Utils/vocabmatch.js';
import WordOrder from '../../Utils/vocabmatch.js';
import DefinitionMatch from '../../Utils/vocabmatch.js';
import categoryData from "../../Components/VocabularyListComps/Data/vocabcolumn.json";
import gapfillData from '../Vocabulary-Exercises/Data/gapfill.json';
import wordorderData from '../../Utils/Data/ordering/wordorder_present.json';
import Video from '../../Components/VideoComps/videocomponent.js';
import videoData from '../Listening-Exercises/Data/video.json';
import Quiz from '../Quizzes/quiz.js';
import quizzesData from '../Quizzes/Data/sidejobsquiz.json';
import RandomQuestionGenerator from '../../Components/DiscussionComps/questiongenerator.js';
import discussionquestionsData from "../../Components/DiscussionComps/Data/discussionquestions.json";
import headerimg from '../../Images/homebakery2.png';

const gossipGapfill = gapfillData.find(item => item.topic === "gossip" && item.type === "gap-fill");

Modal.setAppElement('#root');

const Sidejobs = () => {

    return (
        <div>
            <SEO
                title='Exam Topics - Sidejobs - English Exam Exercises'
                description='A group of English exam exercises centered around the topic of the side jobs'
                name='English Exam Exercises - Side Jobs'
                type='article' 
            />

            <div className="agenda-container">
                <div className="agenda-title">
                    <h1 className="mt-2">Sidejobs</h1>
                </div>

                <div className="grammarimage">
                <img src={headerimg} alt="English Exam Exercises" />
                </div>
                <div className="agenda-languagepoint">Language Point</div>
                
                
                <div className="languagepoint-container">


                        <div>
                        <GrammarExplanation />
                        </div>
                   
                        <div className="agenda-display-grid">
                           <Grammarexercises />
                        </div>
                   
                    

                </div>
                
                <div className="agenda-vocabulary">Quiz</div>              
                   
                <div className="agenda-display-grid">
                    
                    
                    
                    </div>
                    
                    <div>
                    <Quiz quizzesData={quizzesData} /> 
                    </div>
                    
                

                <div className="agenda-reading">Reading</div> 
                <div className="reading-container">
                <div className="agenda-display-grid">
                <Reading readings={readingcompData} title="Why We Didn't Open a Bakery Storefront" />
                    </div>
                </div>
                
                <div className="agenda-discussion">Discussion</div>
                <RandomQuestionGenerator discussionquestions={discussionquestionsData} topic="sidejobs" />
            </div>
        </div>
    );
};

export default Sidejobs;
