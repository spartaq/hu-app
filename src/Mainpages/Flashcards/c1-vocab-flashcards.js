...
function C1Vocabulary() {
  ...
  const setLabels = Object.keys(b2adjectives).map(setKey => {
    const range = b2adjectives[setKey][0].range; // Get the range from the first card of each set
    return { setKey, label: `${setKey} ${range}` };
  });

  useEffect(() => {
    setCards(b2adjectives[currentSet]);
  }, [currentSet]);
  ...
  const handleSetChange = (set) => {
    console.log(`Navigating to: /c1-vocab-flashcards?set=${set}&topic=all`);
    setCurrentSet(set);
    setSelectedTopic('all'); // Reset topic filter when set changes
    setCurrentCardIndex(0);
    setIsFlipped(false);
    setIsMenuOpen(false); // Close menu after selecting a set
    navigate(`/flashcards/c1-vocab-flashcards?set=${set}&topic=all`);
  };
  
  const handleTopicChange = (topic) => {
    console.log(`Navigating to: /c1-vocab-flashcards?set=${currentSet}&topic=${topic}`);
    setSelectedTopic(topic);
    setCurrentCardIndex(0);
    setIsFlipped(false);
    setIsTopicMenuOpen(false); // Close topic menu after selecting a topic
    navigate(`/flashcards/c1-vocab-flashcards?set=${currentSet}&topic=${topic}`);
  };
  ...
  const filteredCards = selectedTopic === 'all'
    ? cards
    : Object.values(b2adjectives).flat().filter(card => card.topic === selectedTopic);
  ...
  return (
    <div>
      <SEO
        title='Vocabulary Flashcards - C1 Vocabulary - English Exam Exercises'
      ...
      <div className={`flashpage-container div1 ${isFullscreen ? 'fullscreen' : ''}`}>
        <div className={`column-right ${isFullscreen ? 'fullscreen' : ''}`}>
          ...
          <div className="flashheader">
            <div className='flashsetnum'>C1 Vocabulary - </div><span className="card-level">{filteredCards[currentCardIndex].range}</span>
      ...
      {filteredCards[currentCardIndex].tip}
          ...
          <div className={`mobile-menu ${isMenuOpen ? 'open' : ''}`}>
              ...
              <div className="set-buttons">
                ...
                {Object.keys(b2adjectives).map((set, i) => (
                  <button
                    key={i}
                    className="flashbtn"
                    onClick={() => handleSetChange(set)}
                  >
                    {`Set ${i + 1} ${b2adjectives[set][0].range}`}
                  </button>
                ))}
