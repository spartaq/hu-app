...
import b2adjectives from './Data/b2adjectives.json';
...
function FlashB2Adj() {
  ...
  const handleSetChange = (set) => {
    console.log(`Navigating to: /b2-adj-flashcards?set=${set}&topic=all`);
    setCurrentSet(set);
    setSelectedTopic('all'); // Reset topic filter when set changes
    setCurrentCardIndex(0);
    setIsFlipped(false);
    setIsMenuOpen(false); // Close menu after selecting a set
    navigate(`/flashcards/b2-adj-flashcards?set=${set}&topic=all`);
  };
  
  const handleTopicChange = (topic) => {
    console.log(`Navigating to: /b2-adj-flashcards?set=${currentSet}&topic=${topic}`);
    setSelectedTopic(topic);
    setCurrentCardIndex(0);
    setIsFlipped(false);
    setIsTopicMenuOpen(false); // Close topic menu after selecting a topic
    navigate(`/flashcards/b2-adj-flashcards?set=${currentSet}&topic=${topic}`);
  };
      ...
      <div className={`flashpage-container ${isFullscreen ? 'fullscreen' : ''}`}>
        <div className={`column-right ${isFullscreen ? 'fullscreen' : ''}`}>
          ...
          <div className="flashheader">
            <div className='flashsetnum'>B2 Adjectives - </div><span className="card-level">{filteredCards[currentCardIndex].range}</span>
