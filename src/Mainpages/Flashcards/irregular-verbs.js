...
import irregularverbs from './Data/irregularverbs.json';
...
function IrregularVerbs() {
  ...
  const setLabels = Object.keys(irregularverbs).map(setKey => {
    const range = irregularverbs[setKey][0].range; // Get the range from the first card of each set
    return { setKey, label: `${setKey} ${range}` };
  });

  useEffect(() => {
    setCards(irregularverbs[currentSet]);
  }, [currentSet]);
  ...
  const handleSetChange = (set) => {
    console.log(`Navigating to: /irregularverbs?set=${set}&topic=all`);
    setCurrentSet(set);
    setSelectedTopic('all'); // Reset topic filter when set changes
    setCurrentCardIndex(0);
    setIsFlipped(false);
    setIsMenuOpen(false); // Close menu after selecting a set
    navigate(`/flashcards/irregularverbs?set=${set}&topic=all`);
  };
  
  const handleTopicChange = (topic) => {
    console.log(`Navigating to: /irregularverbs?set=${currentSet}&topic=${topic}`);
    setSelectedTopic(topic);
    setCurrentCardIndex(0);
    setIsFlipped(false);
    setIsTopicMenuOpen(false); // Close topic menu after selecting a topic
    navigate(`/flashcards/irregularverbs?set=${currentSet}&topic=${topic}`);
  };
  ...
  const filteredCards = selectedTopic === 'all'
    ? cards
    : Object.values(irregularverbs).flat().filter(card => card.topic === selectedTopic);
      ...
      <div className={`flashpage-container ${isFullscreen ? 'fullscreen' : ''}`}>
        <div className={`column-right ${isFullscreen ? 'fullscreen' : ''}`}>
          ...
          <div className="flashheader">
            <div className='flashsetnum'>Irregular Verbs </div><span className="card-level">{filteredCards[currentCardIndex].range}</span>
      ...
      {filteredCards[currentCardIndex].tip}
          ...
          <div className={`mobile-menu ${isMenuOpen ? 'open' : ''}`}>
              ...
              <div className="set-buttons">
                ...
                {Object.keys(irregularverbs).map((set, i) => (
                  <button
                    key={i}
                    className="flashbtn"
                    onClick={() => handleSetChange(set)}
                  >
                    {`Set ${i + 1} ${irregularverbs[set][0].range}`}
                  </button>
                ))}
