import React, { useEffect, useRef, useState } from "react";
import "../../CSS/dialoguePlayer.css";

export default function DialoguePlayer({
  lines = [],
  audioSrc = null,
  subtitleMode = false,
}) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [playing, setPlaying] = useState(false);
  const [ready, setReady] = useState(true); // overlay active initially
  const audioRef = useRef(null);
  const containerRef = useRef(null);

  // --- Sync dialogue to audio timestamps ---
  useEffect(() => {
    if (!audioSrc || !audioRef.current) return;
    const audio = audioRef.current;

    const onTimeUpdate = () => {
      if (!playing) return;
      const currentTimeMs = audio.currentTime * 1000;
      const nextIndex = lines.findIndex(
        (line, i) =>
          currentTimeMs >= line.start &&
          currentTimeMs <= line.end &&
          i !== currentIndex
      );
      if (nextIndex !== -1) setCurrentIndex(nextIndex);
    };

    audio.addEventListener("timeupdate", onTimeUpdate);
    return () => audio.removeEventListener("timeupdate", onTimeUpdate);
  }, [lines, currentIndex, playing]);

  // --- Auto-scroll to active line ---
  useEffect(() => {
    if (containerRef.current) {
      const activeLine = containerRef.current.children[currentIndex];
      if (activeLine) {
        activeLine.scrollIntoView({ behavior: "smooth", block: "center" });
      }
    }
  }, [currentIndex]);

  // --- Play / Pause handler ---
  const handlePlayPause = () => {
    if (!audioRef.current) return;

    if (playing) {
      audioRef.current.pause();
      setPlaying(false);
    } else {
      audioRef.current.play().catch((err) => console.error("Audio play failed:", err));
      setPlaying(true);
    }
  };

  // --- Start from Ready overlay ---
  const handleStart = () => {
    if (!audioRef.current) return;
    audioRef.current.play().catch((err) => console.error("Audio play failed:", err));
    setPlaying(true);
    setReady(false); // hide overlay
  };

  // --- Restart handler ---
  const handleRestart = () => {
    if (audioRef.current) audioRef.current.currentTime = 0;
    setCurrentIndex(0);
    setPlaying(false);
    setReady(true); // show overlay again
  };

  const currentLine = lines[currentIndex] || {};

  return (
    <div className="dialogue-wrapper">
      {audioSrc && <audio ref={audioRef} src={audioSrc} preload="auto" />}

      {/* Overlay Play button */}
      {ready && (
        <div className="dialogue-overlay">
          <button className="dialogue-overlay-button" onClick={handleStart}>
            ▶ Play Dialogue
          </button>
        </div>
      )}

      <div className="dialogue-container" ref={containerRef}>
        {lines.map((line, i) => (
          <div
            key={i}
            className={`dialogue-line ${
              line.position === "right" ? "right" : "left"
            } ${i === currentIndex ? "active" : ""} fade-slide`}
          >
            {line.avatar && <img src={line.avatar} className="avatar" alt="" />}
            <div className="bubble">
              <b>{line.speaker}:</b> {line.text}
            </div>
          </div>
        ))}
      </div>

      {subtitleMode && currentLine && (
        <div className="subtitle-bar fade-in">
          <b>{currentLine.speaker}:</b> {currentLine.text}
        </div>
      )}

      <div className="dialogue-controls">
        <button onClick={handlePlayPause}>{playing ? "Pause" : "Play"}</button>
        <button onClick={handleRestart}>Restart</button>
      </div>
    </div>
  );
}
